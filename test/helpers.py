...
class TestHelpers(object):
    ...
    def build_body_param(param, klassName):
        ...
        if klassName == IncomingNumberRequest:
            return IncomingNumberRequest(
                application_id="TEST_APPLICATION_ID", alias="TEST_ALIAS"
            )
        ...
        if klassName == AccountRequest:
            return AccountRequest(alias="TEST_ALIAS", label="TEST_LABEL")
        if klassName == QueueRequest:
            return QueueRequest(alias="TEST_ALIAS", max_size=10)
        if klassName == UpdateConferenceParticipantRequest:
            return UpdateConferenceParticipantRequest(talk=True, listen=True)
        ...
        if klassName == FilterLogsRequest:
            return FilterLogsRequest(pql="TEST_PQL")
        ...
        if klassName == MessageRequest:
            return MessageRequest(
                _from="TEST_FROM_NUMBER", to="TEST_TO_NUMBER", text="TEST_TEXT"
            )
        return "TEST_" + param.upper()
    ...
    def serialize_body_param(param):
        if (
            isinstance(param, ModelSimple)
            or isinstance(param, ModelNormal)
            or isinstance(param, ModelComposed)
        ):
...
class MockPoolManager(object):
    ...
    def expect_request(self, *args, **kwargs):
        ...
        if method != "GET":
            kwargs.pop("fields")
        if (
            method in ["POST", "PUT", "PATCH", "OPTIONS"]
            and "Content-Type" not in kwargs["headers"].keys()
        ):
