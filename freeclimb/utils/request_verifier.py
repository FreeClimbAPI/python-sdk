...
class RequestVerifier:
    ...
    def verify_request_signature(self, request_body:str, request_header:str, signing_secret:str, tolerance:int=DEFAULT_TOLERANCE):
        self.check_request_body(request_body)
        self.check_request_header(request_header)
        self.check_signing_secret(signing_secret)
        self.check_tolerance(tolerance)
        info = SignatureInformation(request_header)
        self.verify_tolerance(info, tolerance)
        self.verify_signature(info, request_body, signing_secret)
