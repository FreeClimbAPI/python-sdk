...
def lazy_import():
    ...
    from freeclimb.model.transcribe_utterance_all_of_record import (
...
class TranscribeUtterance(ModelComposed):
    ...
    def command():
        ...
        mapping = next(
            (
                mapping
                for mapping, schema in mappings.items()
                if schema == TranscribeUtterance
            ),
            None,
        )
        if mapping == None:
            raise ApiAttributeError(
                "{0} has no mapping '{1}'".format(
                    TranscribeUtterance.__class__.name, "command"
                )
            )
        return mapping
